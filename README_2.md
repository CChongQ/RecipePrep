...
To make nutrient calculation and recipe guidance more reliable, we grounds ingredient nutrition data in the Canadian Nutrient File (CNF), scores recipes with deterministic nutrition rules, and uses few-shot retrieval to provide the model with similar filtered recipes as examples.
...
For repeated local use, copy `.env.example` to `.env` and add your key there.
...
**Important Note!**: Different parts may uses models to process different volumn of data. Please remeber to change model type in each step based on your requirement and budget. The rebuild scripts below use OpenAI models by default.
